...
// List<StockInwardList> stoutList=[];
  ...
  passData(ThemeData theme, BuildContext context, int index) {
    log("StockInward[index].data length:" +
        StockInward[index].data[0].StOutSerialbatchList!.length.toString());
    if (StockInward[index].data.isNotEmpty) {
      ...
      //log("${passdata![0].StOutSerialbatchList!.length}");
      notifyListeners();
  ...
  stkInEditQty(
    ...
    log("serialbatchList MM::" +
        StockInward[index].data[list_i].trans_Qty.toString());
...
// docnumber generatiom
    ...
    for (int i = 0; i < StockInward[index].data.length; i++) {
      ...
      if (StockInward[index].data[i].serialbatchList != null) {
        ...
        for (int l = 0;
            l < StockInward[index].data[i].serialbatchList!.length;
            l++) {
          stInbatch.add(StockInBatchDataDB(
              ...
              itemcode: StockInward[index]
                  ...
                  .itemcode
                  .toString(),
              qty: StockInward[index].data[i].serialbatchList![l].qty == null
                  ? 0
                  : int.parse(StockInward[index]
                      .data[i]
                      .serialbatchList![l]
      ...
      await DBOperation.insertStInBatch(
          db, stInbatch); //  List<Map<String, Object?>> getDB_StOutBatch =
  ...
  addBatchtable(int index, int ik) {
    batchTable = [];
    for (int i = 0;
        i < StockInward[index].data[ik].serialbatchList!.length;
        i++) {
      batchTable!.add(StockInbatch(
        // baseLineNumber: i,
        quantity: double.parse(
            StockInward[index].data[ik].serialbatchList![i].qty.toString()),
        // StockInward[index].data[ik].serialbatchList![i].qty.toString()),
        batchNumberProperty: StockInward[index]
            .data[ik]
            .serialbatchList![i]
            .serialbatch
            .toString(),
  ...
  addOutLinedata(int index) {
    ...
    for (int i = 0; i < StockInward[index].data.length; i++) {
      ...
      StockInwardLines!.add(StockInLineModel(
        ...
        toWarehouseCode: StockInward[index].reqtoWhs.toString(),
        lineNum: i,
        batchNumbers: batchTable!,

        // baseDocentry:
        //     int.parse(StockInward[index].sapbaceDocentry.toString()),
        // baseline: StockInward[index].data[i].baseDocline.toString(),
        // baseType: '1250000001'
      ));
    }
    notifyListeners();
  ...
  postInwardData(
    ...
    PostStkInwardAPi.fromWarehouse = "HOGIT";
    PostStkInwardAPi.toWarehouse = StockInward[index].reqtoWhs.toString();
    ...
    await PostStkInwardAPi.getGlobalData().then((value) async {
      if (value.statusCode! >= 200 && value.statusCode! <= 210) {
        if (value.docEntry != null) {
          ...
          await updateInWrdStkSnaptab(int.parse(docEntryId.toString()),
              int.parse(baseentry.toString()));

          await PostRabitMq2(int.parse(docEntryId.toString()), baseentry,
  ...
  Future getHoldValues(Database db) async {
    ...
    for (int i = 0; i < getDBStInHead.length; i++) {
      ...
      List<Map<String, Object?>> getStkReqHead =
          await DBOperation.getStockHDReq(
              db, int.parse(getOutWrdHead[0]["baseDocentry"].toString()));
...
//---------------------------
        ...
        StIn_Line.add(StockInwardDetails(
            ...
            trans_Qty: getDBStInLine[j]["transferQty"] == null
                ? 0
                : int.parse(getDBStInLine[j]["transferQty"].toString()),
  ...
  passList(List<StockInwardDetails>? data) {
    passdata = data!;
    log("passdata!.length::" +
        passdata![0].StOutSerialbatchList!.length.toString());
  ...
  getStockOutData() async {
    ...
    if (getStockOutValues.isNotEmpty) {
      for (int i = 0; i < getStockOutValues.length; i++) {
        //log("step--2::${getStockOutValues.length}");
        //log('getStockOutValues[i]["reqfromWhs"]:::${getStockOutValues[i]["reqfromWhs"]}');
        ...
        if (getStockOutValues[i]["reqfromWhs"].toString() ==
          ...
          for (int j = 0; j < getStockOutLineData.length; j++) {
            ...
            // AppConstant.branch)
            {
              ...
              StOutSerialbatchList2 = [];
              //log("${getStockOutLineData[j]["docentry"].toString()}:::${getStockOutLineData[j]["baseDocentry"].toString()}");
              ...
              stockDetails.add(StockInwardDetails(
                  ...
                  price: getStockOutLineData[j]["price"] == null
                      ? 0.0
                      : double.parse(
                          getStockOutLineData[j]["price"].toString()),
                  serialBatch: getStockOutLineData[j]["serialBatch"].toString(),
                  taxRate: 0.0,
                  Scanned_Qty:
                      int.parse(getStockOutLineData[j]["scanndQty"].toString()),
                  trans_Qty:
                      int.parse(getStockOutLineData[j]["balqty"].toString()),
                  taxType: "",
                  StOutSerialbatchList: StOutSerialbatchList2));
              print("StOutSerialbatchList2------------------ ::" +
